...
public class TXNetFTPHandler {
	...
	protected byte[] data;
	protected TXNetFTPListener listener;

	public TXNetFTPHandler() {
		initialize();
	...
	public void handlePacket(TXNetFileTransferPacket packet) {
	...
	private boolean verifyInitialization(FileMetaDataTransfer metadata) {
		...
		var maxFileSize = TXNetConfig.getInstance().getMaxFileSize();
